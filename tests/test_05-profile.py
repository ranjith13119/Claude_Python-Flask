...
class TestProfilePageContent:
    ...
    def test_page_shows_transaction_table_with_rows(self, client):
        login(client)
        body = client.get("/profile").get_data(as_text=True)
        assert body.count("profile-table") == 1
        assert "<tr>" in body
        assert "Lunch at cafe" in body or "Groceries for the week" in body

    def test_page_shows_category_breakdown_with_categories(self, client):
        login(client)
        body = client.get("/profile").get_data(as_text=True)
