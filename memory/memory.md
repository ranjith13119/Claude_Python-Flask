...
## 2026-08-14 — Step 11 Investment Tracking (implemented + tested, 199/199)

- User request: add expense + investment categories, capture investments (MF, Stocks, Gold etc.), calculate earnings + expenses + investments. Decisions (asked via questions): profile section like earnings; 7 types MF/Stocks/Gold/Bonds/Crypto/Real Estate/Other; Investment-category expenses DO count in Spent; Invested column added to monthly comparison (Month|Earnings|Spent|Invested|Net).
- db.py: CATEGORIES += "Investment" (8); INVESTMENT_TYPES constant; investments table (id, user_id FK, type, amount REAL, date, note nullable, created_at); create_investment + get_investments_by_user_id (ORDER BY date DESC).
- Seed changed: 9th expense ("Gold coin", Investment, ₹50, today-8) so seed covers all 8 categories. New seed totals: ₹344.64, Shopping top ₹79.99, percents Food22/Transport7/Bills17/Health6/Entertainment6/Shopping23/Other4/Investment15. IMPORTANT: seed-total assertions in test_04/05/06 were updated from COMPUTED values (don't hardcode-guess).
- app.py: extracted parse_amount(amount_raw) -> (amount, error) shared by expense validator, earnings route and new validate_investment_input (one source of truth for amount rules incl. math.isfinite); POST /investments (guard, whitelisted type, strict date not future, note->NULL, error -> render_profile(investment_error=error), success PRG); render_profile(earnings_error=None, investment_error=None); build_monthly_comparison now takes investments and adds invested-by-month (months = union of expenses/earnings/investments, DESC); net stays earnings - spent (investments already in Spent; Invested column informational — no double counting); build_investment_breakdown mirrors category breakdown shape; stats["total_invested"].
- profile.html: 4th stat card Total invested; Investments card (type select, amount, date default current_month-01, note; Date|Type|Amount|Note table; by-type breakdown reusing .breakdown-*; "No investments yet."); Invested column + colspan 4->5.
- CSS: .earnings-* rules converted to shared selectors (.earnings-form,.invest-form / .earnings-table,.invest-table / .earnings-save,.invest-save) + .card-subtitle; vars only.
- Tests: test_11 27/27 (access control, insert + NULL note, all 7 types, 11 invalid variants incl nan/inf, form renders types, stat card, list, breakdown 75/25, empty state, invested column, net unchanged, investment-only month, template rules). Updated: test_01 (categories + count 9), test_04 (₹344.64, >9<, 4 stat cards, 22/23/17), test_05 (₹344.64, pct-22/pct-23, count 9), test_06 (₹344.64, count 9), test_07/08 (8 categories, renamed seven->eight).
- Branch: feature/investments (from feature/crud-earnings — PR #11 for 08-10 still open; PR #12 base main auto-drops the 08-10 commits after #11 squash-merges). Spec .claude/spec/11-investments.md + mirror; plan .claude/plans/11-investments.md + mirror; Logs/11-investments.md.
